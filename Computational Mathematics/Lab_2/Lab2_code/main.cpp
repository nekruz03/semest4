...
int main() {
    ...
    ChordMethod sm;
    cout.precision(20);
    cout << "Do you want to solve equation or system? Type 1 if system, else 2" << std::endl;
    int type;
    cin >> type;
    if (type == 2) {
        cout
        ...
        if (num == 1) {
            ...
            if (!res.success) {
                cout << "no roots" << std::endl;
            } else {
                double ans1 = bs.get_root(polynomyal, -20, 20).ans;
                double ans2 = sm.get_root(polynomyal, -20, 20).ans;
                cout << "Bisectional: " << ans1 << std::endl;
                cout << "Chord: " << ans2 << std::endl;
                cout << "Difference: " << std::abs(ans2 - ans1);
            }
        }
            if (num == 2) {
            ...
            auto system = vector<function<double(vector<double>)>>{func1_system, func2_system,
                                                                                  func3_system};
            auto ans = iterationMethod.solve_system(system, 0.000001);
            for (int i = 0; i < ans.size(); i++) {
                std::cout << "answer" << i + 1 << ": " << ans[i] << std::endl;
            }
        }
