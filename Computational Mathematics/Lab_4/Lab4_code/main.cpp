...
int sin_left = -3;
int sin_right = 4;
function<double(double)> func_sin = [](double x) { return sin(x); };

vector<double> x_polynom;
vector<double> y_polynom;
function<double(double)> func_polynom = [](double i) { return i * i * i - 3 * i * i + 2 * i - 1; };

int polynom_left = -3;
int polynom_right = 4;

vector<double> x_rnd_func;
vector<double> y_rnd_func;
int rnd_func_left = -3;
int rnd_func_right = 4;
function<double(double)> func_rand = [](double i) { return sin(i) * i + 2 - cos(i); };
...
vector<double> interpolated_x;
vector<double> interpolated_y;
NewtonMethod nm;
...
int main() {
    ...
    cout << "if you want to interpolate sin type 1 -> sin(x) " << std::endl;

    cout << "if you want to interpolate polynomial func type 2 -> i^3 - 3 * i^2 + 2i - 1 " << std::endl;

    cout << "if you want to interpolate another func type 3 -> sin(i) * i + 2 - cos(i)" << std::endl;
    int n;
    cin >> n;
